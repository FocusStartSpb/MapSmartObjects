//
//  Colors.swift
//  MapSmartObjects
//
//  Created by Максим Шалашников on 11.01.2020.
//  Copyright © 2020 Максим Шалашников. All rights reserved.
//

import UIKit

enum Colors
{
	static let mainStyle = #colorLiteral(red: 0, green: 0.7444902062, blue: 0.9746511579, alpha: 1)
	static let complementary = #colorLiteral(red: 1, green: 1, blue: 1, alpha: 1)
	static let pinsLimit = UIColor.red
	static let radiusFill = UIColor.green.withAlphaComponent(0.3)
	static let shadows = UIColor.black
	static let carriage = UIColor.gray
}
